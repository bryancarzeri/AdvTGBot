...
if ($TGBot->settings['adminMySQL']) {
    ...
    if ($TGBot->cbdata_text == '/unumber' and $TGBot->botAdmin()) {
        ...
        $private = $TGBot->mdb->prepare("SELECT * FROM $TGBot->table_name WHERE type=?");
        $private->execute(['private']);
        $groups = $TGBot->mdb->prepare("SELECT * FROM $TGBot->table_name WHERE type=?");
        $groups->execute(['group']);
        $supergroup = $TGBot->mdb->prepare("SELECT * FROM $TGBot->table_name WHERE type=?");
        $supergroup->execute(['supergroup']);
        $channel = $TGBot->mdb->prepare("SELECT * FROM $TGBot->table_name WHERE type=?");
    ...
    if (strpos($TGBot->cbdata_text, '/gpost') === 0 and $TGBot->botAdmin()) {
        ...
        if ($t[1] == 'cancel') {
            ...
            $update = $TGBot->mdb->prepare("UPDATE $TGBot->table_name SET action=?, to_update=? WHERE chat_id=?");
    ...
    if ($TGBot->text == '/send' and isset($TGBot->reply_to_message) and $TGBot->botAdmin()) {
        $select = $TGBot->mdb->prepare("SELECT * FROM $TGBot->table_name WHERE chat_id=?");
        ...
        if ($ex[1] == 'g') {
            ...
            $fetchAll = $TGBot->mdb->prepare("SELECT * FROM $TGBot->table_name WHERE type=?");
